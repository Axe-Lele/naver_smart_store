...
export function App() {
  ...
  const progressDetailText = isCollectingProducts
    ? '전용 브라우저 탭에서 현재 페이지 상품을 읽고 있습니다.'
    : isStartingBatch
      ? '선택한 상품 작업을 전용 브라우저 탭으로 보내고 있습니다.'
    : isStoppingBatch
      ? '전용 브라우저 탭에 중단 요청을 보내고 있습니다.'
      : isStopped
  ...
  async function bootApplication(): Promise<void> {
    try {
      ...
      setNotice({
        tone: 'info',
        text: '전용 브라우저를 열고 그 안에서 직접 로그인한 뒤, 상품 조회/수정 화면에서 상품 불러오기를 누르면 됩니다.',
      });
    } catch (error) {
      showError(error);
    } finally {
    ...
    try {
      ...
      setNotice({
        ...
        text: queued.targetClientId
          ? `${describeHybridCommand(type)} 요청을 전용 브라우저 탭으로 보냈습니다.`
  ...
  function scheduleCommandResponseTimeout(
    ...
    pendingCommandTimeoutIdRef.current = window.setTimeout(() => {
      ...
      setNotice({
        tone: 'error',
        text: '중단 요청에 3초 동안 응답이 없습니다. 확장프로그램이 꺼졌거나 전용 브라우저 탭 연결이 끊겼을 수 있습니다. 판매자센터 탭을 새로고침한 뒤 상태를 확인해 주세요.',
      });
      void refreshHybridState();
  ...
  async function handleLoadProducts(): Promise<void> {
    ...
    if (isLoginPage) {
      setNotice({
        tone: 'error',
        text: '전용 브라우저 탭이 아직 로그인 화면입니다. 판매자센터 로그인을 끝낸 뒤 상품 조회/수정 화면에서 다시 눌러 주세요.',
      });
      return;
    }
    ...
    setNotice({
      tone: 'info',
      text: '전용 브라우저 판매자센터 화면에서 상품을 읽고 있습니다. 잠시만 기다려 주세요.',
    });
    ...
    try {
      ...
      setNotice({
        ...
        text: isCurrentLoginPage
          ? `${prefix} 전용 브라우저에서 로그인을 끝낸 뒤 상품 조회/수정 화면을 열어 주세요.`
          : `${prefix} 전용 브라우저의 상품 조회/수정 탭을 열고 Ctrl+R로 새로고침해 주세요.`,
      });
      return false;
    } catch (error) {
      setPendingCommand(null);
  ...
  async function handleOpenSellerCenter(): Promise<void> {
    try {
      ...
      setNotice({
        tone: 'success',
        text: '전용 브라우저를 열었습니다. 열린 브라우저에서 직접 로그인한 뒤 상품 조회/수정 화면을 확인해 주세요.',
      });
    } catch (error) {
      showError(error);
    }
  ...
  async function handleExtensionSetupHelper(): Promise<void> {
    ...
    try {
      ...
      setNotice({
        tone: 'success',
        text:
          '전용 브라우저에서는 확장을 자동 로드합니다. 연결이 안 될 때만 열린 확장 관리 화면에서 압축해제된 확장 프로그램 로드로 폴더를 선택하세요.',
      });
    } catch (error) {
      showError(error);
    }
        ...
        <aside className="operator-side-panel">
          ...
          {!hybridState?.connected ? (
            <section className="side-card compact-side-card">
              <h2>연결되지 않을 때</h2>
              <p className="muted">
                전용 브라우저는 확장을 자동 로드합니다. 연결되지 않으면 상품 조회/수정 탭을 Ctrl+R로 새로고침하세요.
              </p>
...
function toOperatorMessage(message?: string): string {
  ...
  if (value.includes('seller center') || value.includes('판매자센터')) {
    return value.includes('Chrome')
      ? value
      : '전용 브라우저에서 스마트스토어 판매자센터 상품 조회/수정 화면을 열어 주세요.';
  }
  ...
  if (value.includes('Target page') || value.includes('frame was detached')) {
    return '전용 브라우저 탭이 새로고침되거나 이동하면서 연결이 끊겼습니다. 판매자센터 화면을 다시 열고 상품 불러오기를 다시 눌러 주세요.';
  }
