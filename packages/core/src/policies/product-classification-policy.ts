// File: packages/core/src/policies/product-classification-policy.ts
import { z } from 'zod';

import type { ProductStatus } from '../models/product.js';

export const productClassificationSignalsSchema = z.object({
  pageReady: z.boolean(),
  editPageIdentityVisible: z.boolean(),
  notFoundMessageVisible: z.boolean(),
  preorderSectionVisible: z.boolean(),
  preorderSelected: z.boolean(),
  normalSelected: z.boolean(),
  normalOptionVisible: z.boolean(),
  normalOptionDisabled: z.boolean(),
  preorderFieldDisabled: z.boolean(),
  orderPeriodLockMessageVisible: z.boolean(),
  lockBannerVisible: z.boolean(),
  saveButtonVisible: z.boolean(),
  unexpectedLayoutDetected: z.boolean(),
});

export type ProductClassificationSignals = z.output<
  typeof productClassificationSignalsSchema
>;

export interface ProductClassificationDecision {
  status: ProductStatus;
  reason: string;
  signals: ProductClassificationSignals;
}

export function classifyProductBySignals(
  input: ProductClassificationSignals,
): ProductClassificationDecision {
  const signals = productClassificationSignalsSchema.parse(input);

  if (signals.notFoundMessageVisible) {
    return decision(
      signals,
      'NOT_FOUND',
      'Search or edit UI reported no product result.',
    );
  }
